# OpenSpec vocabulary alignment

Findings from a survey of the OpenSpec ecosystem, addressed to
`consolidate-development-workflow` while its stage vocabulary is still
uncommitted.

This is neither `docs/change-queue.md` nor `docs/deferred-work.md`. Both of
those hold work for later; every entry here is a decision for the change
already open, and the decisions are cheap now and expensive once the
vocabulary is in an archived spec. Fold what is accepted into the change,
record what is declined wherever the change records its rejected
alternatives, and delete this file. It has no life after that.
