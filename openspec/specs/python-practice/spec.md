...
The skill SHALL ship as a split structure from its initial version: a `SKILL.md` carrying the core-traps floor and the general-purpose scope statement as always-resident content, and `references/` files carrying deeper or narrower-audience material loaded on demand rather than resident on every trigger — at minimum covering concurrency (GIL false-safety, forgotten `await`, blocking an event loop), imports and module-level state (circular imports, module-level mutable state as a hidden global), typing and tooling (the `ruff`/`mypy` completion discipline), and datetime/numeric pitfalls (naive vs. aware `datetime`, float vs. `Decimal` for money).
