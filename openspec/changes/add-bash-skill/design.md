...
Recorded per task 7.3, for a future edit to `skills/bash/SKILL.md` rather than added to this change — neither is load-bearing for the spec's minimum content, and adding either now would be padding against Decision 6.

- **`for x in $var; do` is a genuine ShellCheck exception to SC2086.** Verified live during task 3.8: an unquoted variable directly after `in` in a for-loop does not trigger SC2086, unlike the same variable in argument position (`echo $var`). The shipped skill's unquoted-expansion example uses argument position specifically to avoid this exception; the exception itself isn't stated in the body. Worth adding if a future edit needs a concrete "ShellCheck doesn't catch everything the trap describes" example, since this is a cleaner one than the three obligations already named.
- **Dash has a separate, narrower quoting bug on `local`/`export` assignment.** Per the SC2155 wiki page: `local e=$1` or `export g=$(cmd)` fails on dash specifically when the right-hand side contains a space and isn't quoted (`local: bad variable name`), independent of the exit-status-masking issue SC2155 already covers. Not included in `## The shebang decides what's guaranteed`, which sticks to the three failure shapes the spec requires (parse-time rejection, execution-only failure, extension inconsistency) — this is a fourth, narrower shape specific to one implementation's assignment parsing, not one of the three.
