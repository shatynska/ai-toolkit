...
**`$CLAUDE_PLUGIN_ROOT` does not resolve at skill runtime** → It is second in a three-entry chain, so failure degrades to the machine-local path rather than breaking the tool. Verified as a task before the skill is written; if it fails, the skill documents `$AI_TOOLKIT_ROOT` as a setup step and nothing else changes.
...
**The delegated initializer writes files the tool never sees** → `openspec init` demonstrably creates project files (it produced `.claude/commands/opsx/` and `.claude/skills/openspec-*` in this repository), and its behavior against a directory that already holds `AGENTS.md` is not established. The never-overwrite guarantee is therefore bounded to the tool's own writes, and task 1.4 enumerates what the delegated command actually touches before implementation. Left unbounded, the change's headline safety property would be false on the adoption path — the path it exists to serve. Note that the `ai-toolkit` end-to-end run cannot detect this, since OpenSpec is already present here and initialization is skipped.
